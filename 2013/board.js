// In the UI, it's consistently "Alexbot".

var TILE_SIZE = 80;
var PIECES_DIR = "pieces/";
var ORIGINAL_SCRATCH_SRC = PIECES_DIR + "scratch.jpg";
var ANNABOT_SRC = PIECES_DIR + "alexbot.jpg";
...
if (qindex < 0) {
  ...
  PREFIX = document.location.href.substring(qindex + 1);
}
var PIC_NAME = PIECES_DIR + PREFIX;
...
function annabot_rampage() {
  if (!seen_annabot_rampage) {
    alert("Oh no!\n\nAlexbot is on a mad rampage, randomly reordering tiles!");
...
function annabot_help() {
  if (!seen_annabot_help) {
    alert("Your cry for help has been answered.\n\n" +
	  "Alexbot will fix things up a bit!");
...
function check_for_success() {
  if (count_wrong_tiles() == 0) {
    alert("Perfect!\n\nNice work. Now try another puzzle ;-)");
  }
}
